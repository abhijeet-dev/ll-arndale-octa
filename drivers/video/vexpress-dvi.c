...
static struct regmap *vexpress_dvimode_reg;
...
static void vexpress_dvi_mode_set(struct fb_info *info, u32 xres, u32 yres)
{
	...
	for (i = 0; i < ARRAY_SIZE(vexpress_dvi_dvimodes); i++) {
		if (vexpress_dvi_dvimodes[i].xres == xres &&
			...
			err = regmap_write(vexpress_dvimode_reg, 0,
					vexpress_dvi_dvimodes[i].mode);
			break;
		}
	}
...
static struct regmap *vexpress_muxfpga_reg;
...
static int vexpress_dvi_mux_set(struct fb_info *info)
{
	...
	if (!info->device->of_node)
		site = vexpress_config_get_master();
	else {
		u32 position, dcc;
		err = vexpress_config_get_topo(info->device->of_node, &site, &position, &dcc);
		if (err)
			return err;
	}

	if (!vexpress_muxfpga_reg)
		return -ENXIO;

	err = regmap_write(vexpress_muxfpga_reg, 0, site);
	if (!err) {
...
static int vexpress_dvi_probe(struct platform_device *pdev)
{
	...
	switch (func) {
	case FUNC_MUXFPGA:
		vexpress_muxfpga_reg =
				devm_regmap_init_vexpress_config(&pdev->dev);
		device_create_file(&pdev->dev, &dev_attr_fb);
		break;
	case FUNC_DVIMODE:
		vexpress_dvimode_reg =
				devm_regmap_init_vexpress_config(&pdev->dev);
		break;
	}
